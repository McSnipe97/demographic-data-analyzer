...
def calculate_demographic_data(print_data=True):
    ...
    allc = df[['native-country', 'salary']]
    p50k  = allc[allc['salary']=='>50K']
    p50k = p50k.groupby('native-country').count()
    # highest_earning_country_percentage = highest_earning_country/df.groupby('native-country')['native-country'].count().max() *100
    # for val in highest_earning_country:
    #     print(val)
    full = allc.groupby('native-country').count()
    p50k = (p50k*100)/full
    p50k = p50k.sort_values(by=['salary'], ascending=False)
    final = pd.Series(p50k['salary'])
    
    highest_earning_country = final.index[0]
    highest_earning_country_percentage = round(final[0], 1)
